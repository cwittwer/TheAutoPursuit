...
import barely_driven_tokens as tt
import requests
import insta
import twit as twit
import logging
...
try:
    ...
    for i,j in site_list.iterrows(): 
        ...
        cl_fs_car = CraigslistForSale(site=j['site'], category=cat_use, 
                                        filters={'bundle_duplicates' : True,'has_image': True, 'search_titles': True #, 'auto_transmission':'manual' 'query': query_use, 
                                                     , 'min_miles':'2000','max_miles':'50000', 'auto_fuel_type':['gas', 'hybrid', 'electric', 'other']
                                                     , 'min_price':'2000','max_year':'2005', 'min_year':'1970'})
        ...
        for result in cl_fs_car.get_results():
            try:
                ...
                if(date_time_use > datetime_limit): #check to see if posting time was within the last 24 hours from when the code was run
                    ...
                    caption = caption+'\n#cars #barelydriven #carforsale #auto'
